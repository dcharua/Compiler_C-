...
def build_table(node, table, returns=False):  
  ...
  if node[0] is 'function_declaration':
    ...
    child = None
    if isDeclared(variable, node):
      if not isVariable(variable, node):
    ...
    if isDeclared(fun, node):
      if not isFunction(fun, node):
        return [fun[1], fun[2]]
...
def tabla(tree, imprime=True):
    table = {
        'content': {
            'input': ['function', 'int', [], { 'content': {} }],
            'output': ['function', 'void', [['variable', 'int']], { 'content': {} }]
        },
        'scope': None
    }
...
def addVariable(node, table):
  variable_id = node[3]
  wasDefined(table, variable_id, node)
  table['content'][variable_id] = [
    'variable',
    node[1],
    node[2]
  ]
...
def addFunction(node, table):
  function_id = node[2]
  wasDefined(table, function_id, node)
  table['content'][function_id] = [
    'function', 
    node[1], 
    [], 
    {
...
def isDeclared(symbol, node):
  if symbol is None:
    error('ERROR variable NOT declared')
    return False
  return True  

def wasDefined(table, id, node):
  if id in table['content']:
    error('ERROR variable already declared')

def isVariable(variable, node):
  if variable[0] != 'variable':
    error('ERROR is not a variable')
    return False
  return True

def isFunction(function, node):
